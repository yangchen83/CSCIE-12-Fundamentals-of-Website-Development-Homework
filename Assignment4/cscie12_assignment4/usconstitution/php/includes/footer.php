  <div id="footer" class="span-24 last">
	  <p>This XHTML version of the United States Constitution was adapted from <a href="http://www.house.gov/">U.S. House of Representatives</a> website at <a href="http://www.house.gov/house/Constitution/Constitution.html">http://www.house.gov/Constitution/Constitution.html</a>. <br/>
	  <a href="http://www.archives.gov/exhibits/charters/constitution.html">Learn more about the United States Constitution at <strong>The National Archives</strong> online exhibit</a>.
	  <br/>
	  <?php
	  ECHO 'The host server is morpheus.dce.harvard.edu and the path is ' .$_SERVER['REQUEST_URI'];
	  ECHO '<br/>';
	  ECHO 'The documents was modified on ' .DATE("F d Y H:s", getlastmod());
	  ?>
	  </p>
  </div>